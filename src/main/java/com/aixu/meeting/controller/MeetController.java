...
public class MeetController {
	...
	public String showOneMeetInfo(@RequestParam("meetId") String meetId,
			...
			for (AixuMeetandroom aixuMeetandroom : mrs) {
				...
				if(hys.indexOf("广东") >= 0) {
					for (AikoRecepemp aikoRecepemp : gdaxRecEmps) {
						...
						//WeChatUtil.weChatPush(aikoRecepemp.getRempEno(), cancelInfo + hysmc+ depm); // 消息推送
					}
	...
	@ResponseBody
	public int getFreeRoomsAndDks(@RequestParam("staTime") String staTime, 
	...
	@ResponseBody
	public int getAudioInfo(@RequestParam("staTime") String staTime,
			@RequestParam("endTime") String endTime,
			HttpServletResponse res) {
		//判断是否登陆
		//checkIfLogin(res);
	...
	public String saveMeet(AixuMeeting meet, HttpServletRequest req, HttpServletResponse res, Model model)
		...
		if (!meet.getMeetType().equals("视频会议")) {
			...
			for (AixuMeetandroom aixuMeetandroom : meetAndRooms) {
				...
				if(hys.indexOf("广东") >= 0) {
					for (AikoRecepemp aikoRecepemp : gdaxRecEmps) {
						System.out.println("前台\n"+aikoRecepemp.getRempEno() + gyxx + fqr + 
								hyzt + sta + end + bhysmc + depm);
						//WeChatUtil.weChatPush(aikoRecepemp.getRempEno(), gyxx + fqr + hyzt + 
